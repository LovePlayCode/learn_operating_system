...
const ALGO_DESCRIPTIONS: Record<AlgorithmType, string> = {
  [AlgorithmType.FIFO]:
    "先来先服务 (FIFO)调度: 非抢占式。严格按照到达顺序执行。简单，但存在“护航效应”，短任务可能被长任务阻塞。",
  [AlgorithmType.RR]:
    "时间片轮转 (RR): 抢占式。每个进程分配固定时间片。公平性好，响应时间短，适用于分时系统。",
  [AlgorithmType.MLFQ]:
    "多级反馈队列: 动态调整。通常包含多个队列，优先级递减，时间片递增。I/O 密集型保持高优先级。",
  [AlgorithmType.SJF]:
    "最短任务优先 (SJF): 非抢占式。选择预估 Burst Time 最短的进程执行到底。平均等待时间最优，但长作业可能面临“饥饿”。",
  [AlgorithmType.SRTF]:
    "最短完成时间优先 (SRTF): SJF 的抢占式版本。如果新到达进程剩余时间更短，则抢占当前进程。吞吐量高。",
  [AlgorithmType.LOTTERY]:
    "比例/彩票调度 (Proportional): 概率性调度。进程持有一定数量彩票，CPU 随机抽取。票数越多，获得 CPU 概率越高，实现比例分配。",
};
    ...
    >
      ...
      {!compact && (
        <div className="text-[10px] opacity-70">
          执行: {p.remainingTime}/{p.burstTime}
        </div>
      )}
      <div
...
const ProgramToProcessView = () => {
  ...
  return (
    <div className="flex flex-col h-full gap-6">
      {/* Controller */}
      <div
        className={`${styles.card} p-6 shrink-0 flex items-center justify-between`}
      >
        <div>
          <h3 className={`font-bold text-lg ${styles.text.primary}`}>
            从程序到进程 (From Program to Process)
          </h3>
        ...
        <div className="flex gap-2">
          <button
            onClick={() => setStep(0)}
            className={styles.button.icon + " p-3"}
          >
            <RotateCcw size={18} />
          </button>
          <button
      ...
      <div className="flex-1 flex gap-8 items-stretch min-h-[500px]">
        {/* 1. DISK (Left) */}
        <div
          className={`w-1/4 flex flex-col items-center justify-center transition-opacity duration-500 ${
            step > 0 ? "opacity-60 grayscale" : "opacity-100"
          }`}
        >
          <div
            className={`w-48 h-64 rounded-xl border-4 flex flex-col items-center justify-center relative shadow-xl ${
              mode === "cute"
                ? "bg-slate-700 border-slate-600"
                : "bg-slate-800 border-slate-700"
            }`}
          >
            <div className="absolute top-0 left-1/2 -translate-x-1/2 -mt-4 bg-slate-200 text-slate-800 px-3 py-1 rounded-full text-xs font-bold border border-slate-300">
              Hard Disk
            </div>
            ...
            {/* File on Disk */}
            <div
              className={`w-32 bg-white rounded-lg p-2 shadow-lg flex flex-col gap-1 transition-transform ${
                isActive("loader")
            ...
            >
              ...
              <div className="h-6 bg-emerald-100 rounded border border-emerald-200 text-[8px] flex items-center justify-center text-emerald-700">
                .text (Code)
              </div>
              <div className="h-4 bg-blue-100 rounded border border-blue-200 text-[8px] flex items-center justify-center text-blue-700">
                .data (Init)
              </div>
              <div className="h-4 bg-slate-100 rounded border border-slate-200 text-[8px] flex items-center justify-center text-slate-400">
                Headers
              </div>
            </div>
            ...
            <>
              ...
              <div className="bg-white p-3 rounded-full shadow-lg border animate-bounce z-10">
                <ArrowRight
                  size={24}
                  className={
                    mode === "cute" ? "text-pink-400" : "text-blue-600"
                  }
                />
              </div>
              <div className="mt-2 text-[10px] font-bold uppercase text-slate-400 bg-white px-2 py-0.5 rounded border">
                加载器 (Loader)
              </div>
            </>
          )}
        ...
        <div className="flex-1 flex flex-col items-center justify-center">
          <div
            className={`w-full max-w-md h-[450px] rounded-[2rem] border-4 relative flex flex-col items-center pt-10 shadow-2xl transition-all ${
              mode === "cute"
          ...
          >
            ...
            {isActive("mem-static") ? (
              <div className="w-64 flex-1 flex flex-col gap-1 mb-8 animate-in zoom-in duration-500">
                ...
                {/* Stack */}
                <div
                  className={`transition-all duration-700 h-24 rounded border-2 flex flex-col items-center justify-center text-xs font-bold relative ${
                    isActive("mem-dynamic")
                      ? mode === "cute"
                ...
                >
                  ...
                  <div className="absolute bottom-1 right-2 text-[8px] opacity-50">
                    ⬇ 向下增长
                  </div>
                </div>
                ...
                {/* Heap */}
                <div
                  className={`transition-all duration-700 h-16 rounded border-2 flex flex-col items-center justify-center text-xs font-bold relative ${
                    isActive("mem-dynamic")
                      ? mode === "cute"
                ...
                >
                  ...
                  <div className="absolute top-1 right-2 text-[8px] opacity-50">
                    ⬆ 向上增长
                  </div>
                </div>
                ...
                {/* Data */}
                <div
                  className={`h-10 rounded border-2 flex items-center justify-center text-xs font-bold ${
                    mode === "cute"
                ...
                {/* Text */}
                <div
                  className={`h-16 rounded-b-lg border-2 flex flex-col items-center justify-center text-xs font-bold ${
                    mode === "cute"
            ...
            {isActive("pcb") && (
              <div className="absolute -right-16 top-20 animate-in slide-in-from-left-4 duration-500">
                <div
                  className={`w-32 bg-white p-3 rounded-xl border-2 shadow-lg ${
                    mode === "cute" ? "border-pink-300" : "border-slate-600"
                  }`}
                >
                  ...
                  <div className="space-y-1 text-[9px] font-mono text-slate-600">
                    <div className="flex justify-between">
                      <span>PID:</span> <b>101</b>
                    </div>
                    <div className="flex justify-between">
                      <span>State:</span>{" "}
                      <b className="text-green-500">READY</b>
                    </div>
                    <div className="flex justify-between">
                      <span>PC:</span> <b>0x0804800</b>
                    </div>
                  </div>
                </div>
                {/* Connector Line */}
          ...
          {isActive("cpu") && (
            <div className="mt-6 flex items-center gap-4 animate-in slide-in-from-bottom-4">
              ...
              <div
                className={`flex items-center gap-3 px-6 py-3 rounded-2xl shadow-lg text-white ${
                  mode === "cute"
      ...
      >
        <h4 className="font-bold text-sm mb-1 flex items-center gap-2">
          <BookOpen
            size={16}
            className={mode === "cute" ? "text-pink-400" : "text-blue-500"}
          />
          原理说明
        </h4>
        <p className="text-xs leading-relaxed opacity-80">{steps[step].desc}</p>
      </div>
    </div>
  );
};
...
const PCBStructureView = () => {
  ...
  const [activeSection, setActiveSection] =
    useState<keyof typeof PCB_KNOWLEDGE>("INTRO");
  ...
  const pcbData = [
    {
      ...
      pc: 0x00401a,
      sp: 0x7fff0,
      regs: { ax: 0x1a, bx: 0x00, cx: 0xff },
      files: ["stdin", "stdout", "sys.log"],
    },
    {
      id: 102,
      name: "Chrome_Tab",
    ...
    },
    {
      ...
      pc: 0x0091cc,
      sp: 0x7ffc8,
      regs: { ax: 0xee, bx: 0xee, cx: 0x01 },
      files: ["project.ts", "node_modules"],
    },
    {
      id: 108,
      name: "Spotify",
      ...
      pc: 0x002011,
      sp: 0x7ff10,
      regs: { ax: 0x11, bx: 0x44, cx: 0x22 },
      files: ["song.mp3", "audio_out"],
    },
  ];
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6 h-full min-h-[600px]">
      {/* Left: Process List */}
      <div
        className={`${styles.card} p-4 w-full lg:w-56 shrink-0 flex flex-col gap-3`}
      >
        <h3
          className={`font-bold flex items-center gap-2 ${styles.text.primary} text-sm`}
        >
        ...
        <div className="flex-1 overflow-y-auto space-y-2 pr-1 h-32 lg:h-auto">
          {pcbData.map((p) => (
            ...
            >
              <div>
                <div className={`font-bold text-xs ${styles.text.primary}`}>
                  {p.name}
                </div>
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="relative max-w-xl mx-auto flex flex-col gap-4">
          ...
          <div
            className={`${
              styles.card
            } overflow-hidden border-4 relative transition-all duration-300 ${
              mode === "cute" ? "border-pink-200" : "border-slate-700"
            }`}
          >
            {/* Header (State) */}
            <div
              onClick={() => setActiveSection("STATE")}
              className={`cursor-pointer p-4 border-b-2 flex justify-between items-center transition-colors ${
                activeSection === "STATE" ? "bg-opacity-100" : "bg-opacity-90"
              } ${
                mode === "cute"
            ...
            >
              <div>
                ...
                <p className="text-[10px] opacity-70 font-mono">
                  Kernel Addr: 0xC000{activeProcess.id}
                </p>
              </div>
              <div
                className={`px-3 py-1 rounded-lg font-bold text-xs shadow-sm ${getColorByState(
                  activeProcess.state
                )}`}
              >
                {activeProcess.state}
            ...
            <div className="p-6 grid gap-6">
              ...
              <div
                ...
                className={`relative group cursor-pointer p-2 -m-2 rounded-xl border-2 border-transparent transition-all ${
                  activeSection === "ID"
                    ? mode === "cute"
              ...
              >
                ...
                <h4 className="text-xs font-bold uppercase text-purple-500 mb-2 pl-3">
                  标识符 (Identifiers)
                </h4>
                <div className="grid grid-cols-2 gap-4 pl-2">
                  <div
                    className={`p-2 rounded-lg border flex justify-between items-center bg-white ${
                      mode === "cute" ? "border-purple-100" : "border-slate-200"
                    }`}
                  >
              ...
              >
                ...
                <h4 className="text-xs font-bold uppercase text-blue-500 mb-2 pl-3">
                  CPU 上下文 (Registers)
                </h4>
                ...
                <div className={`pl-2`}>
                  <div className="grid grid-cols-4 gap-2 mb-2">
                    {["AX", "BX", "CX", "DX"].map((reg, i) => (
                      <div
                        key={reg}
                        className="text-center bg-white border rounded py-1 shadow-sm text-[10px] font-mono"
                      >
                        <span className="text-slate-400 block text-[8px]">
                          {reg}
                        </span>
                        {i === 0
                          ? `0x${activeProcess.regs.ax
                              .toString(16)
                              .toUpperCase()}`
                          : i === 1
                          ? `0x${activeProcess.regs.bx
                              .toString(16)
                              .toUpperCase()}`
                          : i === 2
                          ? `0x${activeProcess.regs.cx
                              .toString(16)
                              .toUpperCase()}`
                          : "0x00"}
                      </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div
                  ...
                  className={`relative cursor-pointer p-2 -m-2 rounded-xl border-2 border-transparent transition-all ${
                    activeSection === "MEM"
                      ? mode === "cute"
                ...
                >
                  ...
                  <h4 className="text-xs font-bold uppercase text-emerald-500 mb-2 pl-3">
                    内存描述符 (mm)
                  </h4>
                ...
                >
                  ...
                  <h4 className="text-xs font-bold uppercase text-orange-500 mb-2 pl-3">
                    调度信息 (Sched)
                  </h4>
                  <div className="bg-white p-2 rounded border space-y-1 pl-3">
              ...
              >
                ...
                <h4 className="text-xs font-bold uppercase text-sky-500 mb-2 pl-3">
                  文件表 (FD Table)
                </h4>
                <div className="flex flex-wrap gap-1 pl-2">
                  {activeProcess.files.map((f, i) => (
                    <div
                      key={i}
                      className="px-2 py-1 rounded border text-[10px] bg-white flex items-center gap-1 shadow-sm"
                    >
                  ...
                  <div className="px-2 py-1 rounded border border-dashed text-[10px] text-slate-400 bg-white opacity-60">
                    ...
                  </div>
                </div>
              </div>
      ...
      {/* Right: Info Panel */}
      <div
        className={`w-full lg:w-72 shrink-0 flex flex-col transition-all duration-300 ${
          styles.card
        } overflow-hidden border-2 ${
          mode === "cute" ? "border-pink-200" : "border-slate-200"
        }`}
      >
        <div
        ...
        <div className="p-5 flex-1 overflow-y-auto">
          <div className="mb-6">
            ...
            <p
              className={`text-sm leading-relaxed ${styles.text.primary} text-justify`}
            >
              {info.desc}
            </p>
          </div>
                ...
                >
                  <div
                    className={`mt-0.5 w-1.5 h-1.5 rounded-full shrink-0 ${
                      mode === "cute" ? "bg-pink-400" : "bg-blue-500"
                    }`}
                  ></div>
...
const LifecycleView = () => {
  ...
  return (
    <div className="flex flex-col min-h-[700px] lg:h-full gap-6">
      <div
        className={`${styles.card} p-6 h-[500px] lg:flex-1 relative overflow-hidden flex flex-col items-center justify-center`}
      >
        ...
        <div className="relative w-full max-w-2xl h-[400px]">
          {states.map((s) => (
            <div
              key={s.id}
              className={`absolute transform -translate-x-1/2 -translate-y-1/2 px-6 py-4 rounded-xl border-2 font-bold transition-all duration-500 flex flex-col items-center ${
                s.color
              } ${
                demoProc === s.id
                  ? "scale-110 shadow-xl ring-4 ring-offset-2 ring-blue-200 z-10"
      ...
      <div className={`${styles.card} p-6 shrink-0`}>
        <h3 className={`font-bold mb-4 ${styles.text.primary}`}>
          进程状态控制
        </h3>
        <div className="flex flex-wrap gap-4 justify-center">
          {demoProc === ProcessState.NEW && (
            <button
              onClick={() => transition(ProcessState.READY)}
          ...
          {demoProc === ProcessState.READY && (
            <button
              onClick={() => transition(ProcessState.RUNNING)}
              className={styles.button.primary + " px-6 py-2"}
            >
              调度 (Dispatch)
            </button>
          ...
          {demoProc === ProcessState.RUNNING && (
            <>
              ...
              <button
                onClick={() => transition(ProcessState.BLOCKED)}
                className={styles.button.secondary + " px-6 py-2"}
              >
                I/O 请求 (Event Wait)
              </button>
              <button
                onClick={() => transition(ProcessState.TERMINATED)}
                className="bg-red-500 hover:bg-red-600 text-white rounded-xl px-6 py-2 shadow transition-colors"
              >
                退出 (Exit)
              </button>
            </>
          ...
          {demoProc === ProcessState.TERMINATED && (
            <button
              onClick={() => transition(ProcessState.NEW)}
              className={styles.button.secondary + " px-6 py-2"}
            >
              重置进程 (Reset)
            </button>
...
const SchedulerView = () => {
  ...
  const runStep = () => {
    ...
    const prevProcId =
      lastSlice && lastSlice.endTime === time ? lastSlice.processId : null;
    let sliceExpired = false;
    if (prevProcId !== null) {
      let duration = 0;
      for (let i = ganttChart.length - 1; i >= 0; i--) {
        if (ganttChart[i].processId === prevProcId)
          duration += ganttChart[i].endTime - ganttChart[i].startTime;
        else break;
      }
    ...
    if (algorithm === AlgorithmType.FIFO) {
      ...
      if (prevProc && prevProc.remainingTime > 0) currentProc = prevProc;
        ...
        currentProc =
          [...nextQueue].sort((a, b) => a.burstTime - b.burstTime)[0] || null;
    } else if (algorithm === AlgorithmType.SRTF) {
      currentProc =
        [...nextQueue].sort((a, b) => a.remainingTime - b.remainingTime)[0] ||
        null;
    } else if (algorithm === AlgorithmType.MLFQ) {
      currentProc =
        [...nextQueue].sort((a, b) => a.priority - b.priority)[0] || null;
    } else if (algorithm === AlgorithmType.LOTTERY) {
      const prevProc = processes.find((p) => p.id === prevProcId);
      if (prevProc && prevProc.remainingTime > 0 && !sliceExpired)
    ...
    if (currentProc) {
      const updatedProc = {
        ...
        startTime:
          currentProc.startTime === null ? time : currentProc.startTime,
      ...
      if (updatedProc.remainingTime === 0) {
        ...
        if (algorithm === AlgorithmType.RR) {
          let duration = 1;
          for (let i = ganttChart.length - 1; i >= 0; i--) {
            if (ganttChart[i].processId === currentProc!.id)
              duration += ganttChart[i].endTime - ganttChart[i].startTime;
            else break;
          }
  ...
  // Stats
  const finishedProcesses = processes.filter(
    (p) => p.remainingTime === 0 && p.completionTime !== null
  );
  const avgTurnaround =
    finishedProcesses.length > 0
      ? (
          finishedProcesses.reduce(
            (acc, p) => acc + (p.completionTime! - p.arrivalTime),
            0
  ...
  const avgWaiting =
    finishedProcesses.length > 0
      ? (
          finishedProcesses.reduce(
            (acc, p) => acc + (p.completionTime! - p.arrivalTime - p.burstTime),
            0
          ) / finishedProcesses.length
        ).toFixed(1)
      : "--";
  ...
  return (
    <div className="flex flex-col gap-6">
      {/* Toolbar */}
      <div
        className={`${styles.card} p-4 flex flex-wrap items-center justify-between gap-4 shrink-0`}
      >
        <div className="flex items-center gap-4">
          <div className="flex flex-col">
            ...
            >
              ...
              <option value={AlgorithmType.LOTTERY}>
                彩票/比例调度 (Lottery)
              </option>
          ...
          {(algorithm === AlgorithmType.RR ||
            algorithm === AlgorithmType.LOTTERY) && (
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={
              styles.button.primary + " px-4 py-2 flex items-center gap-2"
            }
          >
          ...
          <button
            onClick={addProcess}
            className={`${styles.button.secondary} px-4 py-2 flex items-center gap-2 border-dashed border-2`}
          >
            <Plus size={16} /> 添加进程
          </button>
        </div>
      ...
      {/* Algorithm Info */}
      <div
        className={`px-4 py-3 rounded-xl border text-xs flex gap-2 items-start shrink-0 ${
          mode === "cute"
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <div className="w-full lg:w-72 flex flex-col gap-6 shrink-0">
          ...
          <div className={`${styles.card} p-4 flex flex-col h-72`}>
            <h4
              className={`font-bold mb-3 flex items-center gap-2 text-sm ${styles.text.primary}`}
            >
        ...
        <div className="flex-1 flex flex-col gap-6 min-w-0">
          <div className={`${styles.card} p-6 flex flex-col min-h-[500px]`}>
            <h4
              className={`font-bold mb-4 flex items-center gap-2 ${styles.text.primary}`}
            >
              <Activity size={16} /> CPU 调度图 (Gantt Chart)
              <span className="text-xs font-normal bg-slate-100 px-2 py-1 rounded ml-2 font-mono">
                当前时间: {time}s
              </span>
            </h4>

            <div
              className="relative h-24 w-full bg-slate-50 rounded-xl overflow-x-auto flex border border-slate-200 items-stretch"
              ...
              {ganttChart.map((slice, i) => {
                ...
                return (
                  <div
                    ...
                    style={{ flex: duration }}
                    className={`${
                      slice.processId !== null ? slice.color : "bg-slate-100"
                    } border-r border-white/20 relative group min-w-[20px] transition-all flex items-center justify-center hover:opacity-90 cursor-pointer shrink-0`}
                    onMouseEnter={(e) =>
                      setTooltip({
                        slice,
                    ...
                    onClick={() =>
                      ...
                      setSelectedProcId(slice.processId)
                    }
                  >
                    {slice.processId !== null && (
                      <div className="text-[10px] font-bold text-white pointer-events-none truncate px-1">
                        P{slice.processId}
                      </div>
                    )}
                  </div>
              ...
              >
                <div className="bg-slate-800 text-white text-[10px] p-2 rounded shadow-xl whitespace-nowrap">
                  <div className="font-bold border-b border-slate-600 pb-1 mb-1">
                    Process P{tooltip.slice.processId}
                  </div>
                  <div>
                    Start: {tooltip.slice.startTime}s | End:{" "}
                    {tooltip.slice.endTime}s
                  </div>
                  {tooltip.slice.priority !== undefined && (
                    <div>Priority: Q{tooltip.slice.priority}</div>
                  )}
                  {tooltip.slice.tickets !== undefined && (
                    <div>Tickets: {tooltip.slice.tickets}</div>
                  )}
                </div>
...
export const ProcessView: React.FC = () => {
  ...
  const [tab, setTab] = useState<
    "lifecycle" | "scheduler" | "pcb" | "creation"
  >("creation");

  return (
    <div
      className={`flex flex-col h-full p-6 gap-6 overflow-y-auto ${styles.bg}`}
    >
