...
// --- New Component: CowFileSystemDemo (ZFS/Btrfs Style) ---
const CowFileSystemDemo = () => {
  const { styles, mode } = useTheme();
  
  type Step = 'IDLE' | 'ALLOC' | 'WRITE' | 'COPY_META' | 'SWAP_ROOT' | 'CRASHED';
  const [step, setStep] = useState<Step>('IDLE');
  const [crashStep, setCrashStep] = useState<Step | null>(null);

  // States
  // 0: Old Data (Valid)
  // 1: New Data (Dirty/Allocated)
  // 2: Garbage (Old Data after swap)
  const [dataBlockStatus, setDataBlockStatus] = useState<0 | 1 | 2>(0); 
  const [inodeStatus, setInodeStatus] = useState<0 | 1 | 2>(0);
  const [rootPointer, setRootPointer] = useState<'OLD' | 'NEW'>('OLD');

  const reset = () => {
    setStep('IDLE');
    setCrashStep(null);
    setDataBlockStatus(0);
    setInodeStatus(0);
    setRootPointer('OLD');
  };

  const next = () => {
    if (step === 'IDLE') setStep('ALLOC');
    else if (step === 'ALLOC') { setStep('WRITE'); setDataBlockStatus(1); }
    else if (step === 'WRITE') { setStep('COPY_META'); setInodeStatus(1); }
    else if (step === 'COPY_META') { setStep('SWAP_ROOT'); setRootPointer('NEW'); setDataBlockStatus(0); setInodeStatus(0); } // Visualize as committed
  };
  ...
  const recover = () => {
    // Based on where we crashed, determine state
    if (crashStep === 'SWAP_ROOT') {
       // Too late to crash, already swapped (conceptually) or crash during swap?
       // If crash happened *before* atomic swap finished, we revert.
       // Here we assume crash happens *during* the step visual.
       // In COW, the root pointer update is atomic. Either it happened or it didn't.
       alert("崩溃发生在原子更新之后：系统处于新状态。数据无损。");
       setRootPointer('NEW');
    } else {
       alert("崩溃发生在根节点更新之前：旧树完全有效。新分配的块成为垃圾（GC回收）。数据无损（旧版）。");
       // Revert to old
       setRootPointer('OLD');
       setDataBlockStatus(0); 
       setInodeStatus(0);
  ...
  return (
    <div className="flex flex-col h-full gap-6 p-4">
       <div className={`${styles.card} p-6 flex flex-col md:flex-row justify-between items-center gap-4`}>
          <div>
             <h3 className={`font-bold text-lg ${styles.text.primary} flex items-center gap-2`}>
               <GitBranch size={20} className="text-orange-500"/> 写时复制 (Copy-on-Write / COW)
             </h3>
             <p className={`text-sm ${styles.text.secondary}`}>
               ZFS / Btrfs 的核心机制。永远不覆盖旧数据，而是写入新位置，最后原子更新根指针。
             </p>
          </div>
          <div className="flex gap-2">
             <button onClick={triggerCrash} disabled={step === 'IDLE' || step === 'SWAP_ROOT' || step === 'CRASHED'} className="bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-lg font-bold shadow-sm disabled:opacity-50">
               <Zap size={16}/> 模拟断电
             </button>
             {step === 'CRASHED' ? (
               <button onClick={recover} className={`${styles.button.primary} px-6 py-2 flex items-center gap-2`}>
                 <RotateCcw size={16}/> 恢复文件系统
               </button>
             ) : (
               <button onClick={step === 'SWAP_ROOT' ? reset : next} className={`${styles.button.primary} px-6 py-2 flex items-center gap-2`}>
                 {step === 'SWAP_ROOT' ? <RotateCcw size={16}/> : <ArrowRight size={16}/>}
                 {step === 'SWAP_ROOT' ? "重置" : "下一步"}
               </button>
             )}
          </div>
       </div>
       ...
       <div className="flex-1 flex flex-col items-center justify-center relative min-h-[400px]">
          ...
          {/* Status Bar */}
          <div className="flex gap-1 mb-8 w-full max-w-2xl">
             {['Idle', '1. Alloc New Block', '2. Write Data', '3. Copy Metadata', '4. Atomic Swap'].map((label, i) => {
                const isActive = (step === 'IDLE' && i===0) || (step === 'ALLOC' && i===1) || (step === 'WRITE' && i===2) || (step === 'COPY_META' && i===3) || (step === 'SWAP_ROOT' && i===4);
                return (
                  <div key={i} className={`flex-1 h-2 rounded-full transition-all ${isActive ? 'bg-blue-500 scale-y-150' : 'bg-slate-200'}`} title={label}></div>
                )
             })}
          </div>
          ...
          <div className="flex items-start justify-center gap-16 relative">
             ...
             <div className="flex flex-col items-center z-20">
                <div className={`w-24 h-24 rounded-full border-4 flex flex-col items-center justify-center shadow-xl transition-all ${mode === 'cute' ? 'bg-indigo-100 border-indigo-300' : 'bg-slate-800 border-slate-600 text-white'}`}>
                   <Anchor size={32}/>
                   <div className="text-[10px] font-bold mt-1">Uberblock</div>
                </div>
                {/* Pointer Line */}
                <div className={`w-1 h-16 transition-all duration-500 origin-top ${rootPointer === 'OLD' ? 'rotate-[30deg] bg-slate-400' : '-rotate-[30deg] bg-green-500 scale-x-150'}`}></div>
             </div>

             {/* Trees Container */}
             <div className="absolute top-32 flex justify-center gap-32 w-full">
                
                {/* LEFT: Old Tree */}
                <div className={`flex flex-col items-center transition-all duration-500 ${rootPointer === 'NEW' ? 'opacity-30 grayscale' : 'opacity-100'}`}>
                   {/* Inode Old */}
                   <div className={`w-32 p-3 rounded-xl border-2 flex items-center gap-2 mb-8 bg-white ${rootPointer === 'OLD' ? 'border-slate-400 shadow-md' : 'border-slate-200 border-dashed'}`}>
                      <FileJson size={20} className="text-slate-500"/>
                      <div className="text-xs">
                         <div className="font-bold">Inode A (v1)</div>
                         <div className="text-[9px] text-slate-400">Ptr -> Block 1</div>
                      </div>
                   </div>
                   {/* Line */}
                   <div className="w-0.5 h-8 bg-slate-300 mb-2"></div>
                   {/* Data Old */}
                   <div className={`w-24 h-24 rounded-xl border-2 flex flex-col items-center justify-center bg-slate-100 border-slate-300 text-slate-500`}>
                      <div className="font-bold text-xl">Data v1</div>
                      <div className="text-[10px]">Block #100</div>
                   </div>
                </div>

                {/* RIGHT: New Tree (Shadow until commit) */}
                <div className={`flex flex-col items-center transition-all duration-500`}>
                   
                   {/* Inode New */}
                   <div className={`w-32 p-3 rounded-xl border-2 flex items-center gap-2 mb-8 transition-all ${
                      step === 'COPY_META' || step === 'SWAP_ROOT' 
                        ? 'opacity-100 scale-100 bg-white border-green-500 shadow-lg' 
                        : 'opacity-0 scale-90 translate-y-4'
                   }`}>
                      <FileJson size={20} className="text-green-600"/>
                      <div className="text-xs">
                         <div className="font-bold">Inode A' (v2)</div>
                         <div className="text-[9px] text-slate-400">Ptr -> Block 2</div>
                      </div>
                   </div>
                   
                   {/* Line */}
                   <div className={`w-0.5 h-8 bg-green-400 mb-2 transition-opacity ${step === 'COPY_META' || step === 'SWAP_ROOT' ? 'opacity-100' : 'opacity-0'}`}></div>

                   {/* Data New */}
                   <div className={`w-24 h-24 rounded-xl border-2 flex flex-col items-center justify-center transition-all ${
                      step === 'IDLE' 
                        ? 'opacity-0' 
                        : (step === 'ALLOC' ? 'border-dashed border-blue-300 bg-blue-50 opacity-50' : 'border-green-500 bg-green-50 text-green-700 shadow-md opacity-100')
                   }`}>
                      <div className="font-bold text-xl">Data v2</div>
                      <div className="text-[10px]">Block #200</div>
                      {step === 'ALLOC' && <div className="absolute text-[9px] bg-blue-100 px-1 rounded">Allocating...</div>}
                   </div>
          ...
          <div className={`mt-auto p-4 rounded-xl border max-w-2xl text-center text-sm ${
             step === 'CRASHED' ? 'bg-red-50 border-red-200 text-red-600' : 'bg-slate-50 border-slate-200 text-slate-600'
          }`}>
             <strong>当前状态：</strong>
             {step === 'IDLE' && "系统空闲。所有指针指向旧版本 (v1)。"}
             {step === 'ALLOC' && "分配新块。我们在空闲区域分配 Block #200，完全不触碰旧数据 #100。"}
             {step === 'WRITE' && "写入数据。新数据写入 Block #200。此时若崩溃，新块被视为垃圾，旧树完好无损。"}
             {step === 'COPY_META' && "元数据冒泡。创建新 Inode A' 指向新块 #200。这发生在内存/磁盘的新位置。"}
             {step === 'SWAP_ROOT' && "原子交换 (Atomic Swap)。Uberblock 指针瞬间切换到 Inode A'。事务完成！旧块 #100 变为垃圾。"}
             {step === 'CRASHED' && "系统崩溃！请点击恢复查看一致性结果。"}
          </div>
...
export const FileView: React.FC = () => {
  const { styles, mode } = useTheme();
  const [tab, setTab] = useState<'fd' | 'links' | 'raid' | 'journal' | 'cow_fs' | 'device' | 'disk'>('fd');
    ...
    <div className={`flex flex-col h-full p-6 gap-6 ${styles.bg}`}>
       <div className="flex justify-center shrink-0">
         <div className={`p-1.5 rounded-2xl flex gap-1.5 border shadow-sm flex-wrap justify-center ${mode === 'cute' ? 'bg-white border-pink-100' : 'bg-slate-200 border-slate-300'}`}>
           ...
           <TabButton active={tab === 'cow_fs'} onClick={() => setTab('cow_fs')} label="写时复制 COW" mode={mode} icon={<GitBranch size={14}/>}/>
