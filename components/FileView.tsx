...
// ... (imports remain the same, ensure all are present)
...
const DiskStructureView = () => {
  ...
  // Manual Seek Only
  const seekToTrack = (trackIdx: number) => {
    if (status !== 'IDLE' && status !== 'SEEK') return;
    if (armPos === trackIdx) {
      addLog(`磁头已在磁道 #${trackIdx}`);
      return;
    }
    
    setStatus('SEEK');
    addLog(`1. 寻道: 手动移动磁头到磁道 #${trackIdx}`);
    setArmPos(trackIdx);
    
    setTimeout(() => {
      setStatus('IDLE');
      addLog(`寻道完成: 磁头就绪 (Track #${trackIdx})`);
    }, 800);
  };
  ...
  const readSector = (blockId: number) => {
    ...
    if (diskType === 'hdd') {
      ...
      // Function to proceed to rotation after seek
      const startRotation = () => {
      ...
      if (armPos !== track) {
        ...
        setTimeout(startRotation, 800);
      } else {
        // Already on track, skip seek time (or minimal)
        addLog(`1. 寻道: 磁头已在磁道 #${track} (无需移动)`);
        startRotation();
      }
          ...
          <div className={`${styles.card} flex-1 p-8 flex flex-col items-center justify-center bg-slate-50/50 relative overflow-hidden`}>
             ...
             {diskType === 'hdd' ? (
               <div className="relative w-[320px] h-[320px] flex items-center justify-center">
                  ...
                  >
                     {/* Draw Tracks (Circles) - Now Clickable for Seeking */}
                     {[0, 1, 2].map(t => {
                       const size = 300 - (t * 80); // 300(r150), 220(r110), 140(r70)
                       const isHovered = hoverTrack === t;
                       const isCurrentArm = armPos === t;
                       return (
                         <div 
                           key={t} 
                           onClick={(e) => { e.stopPropagation(); seekToTrack(t); }}
                           onMouseEnter={() => setHoverTrack(t)}
                           onMouseLeave={() => setHoverTrack(null)}
                           className={`absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 rounded-full border-2 transition-all cursor-pointer z-0
                             ${isHovered ? 'border-blue-400 bg-blue-50/20' : 'border-slate-300/50'}
                             ${isCurrentArm ? 'border-orange-300/80 shadow-[0_0_0_1px_rgba(251,146,60,0.4)]' : ''}
                           `} 
                     ...
                     {/* Inner boundary for innermost track (visual hole) */}
                     <div className="absolute border border-slate-300/50 rounded-full left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 pointer-events-none" style={{ width: 60, height: 60 }}></div>
                     ...
                     {Array.from({ length: TRACKS * SECTORS_PER_TRACK }).map((_, i) => {
                        ...
                        return (
                          <button 
                            key={i}
                            onClick={(e) => { e.stopPropagation(); readSector(i); }}
                  ...
                  <div className="absolute top-1/2 left-1/2 w-[320px] h-10 pointer-events-none z-30" 
                     ...
                     {/* Arm Body - Extended to connect head and pivot */}
                     <div className="absolute left-4 right-10 top-1/2 -translate-y-1/2 h-6 bg-gradient-to-r from-slate-400 to-slate-300 shadow-lg" style={{ clipPath: 'polygon(0 40%, 100% 20%, 100% 80%, 0 60%)' }}></div>
                     ...
                     {/* Read Head (The Tip) */}
                     <div className="absolute left-0 top-1/2 -translate-y-1/2 w-6 h-8 flex items-center justify-center transition-all duration-300">
                        <div className={`w-4 h-6 rounded border border-slate-600 shadow-sm transition-colors ${status === 'READ' ? 'bg-green-400 shadow-[0_0_10px_rgba(74,222,128,0.8)]' : status === 'SEEK' ? 'bg-orange-400' : 'bg-slate-200'}`}></div>
                        {/* Laser/Read Point indicator */}
                        <div className={`absolute bottom-0 w-0.5 h-3 ${status === 'READ' ? 'bg-green-500 h-6 w-1' : 'bg-red-500 opacity-50'} transition-all`}></div>
                     </div>
                  </div>
                ...
                <div className="text-xs text-slate-600 leading-relaxed mb-4">
                   {diskType === 'hdd' ? (
                     <ul className="list-disc list-inside space-y-1">
                        <li><strong>点击磁道圆环:</strong> 手动寻道 (Seek)，移动磁头。</li>
                        <li><strong>点击扇区数字:</strong> 完整读取 (Seek + Rotate + Read)。</li>
                     </ul>
                   ) : '点击左侧 Flash 颗粒 (0-63) 模拟电子寻址。'}
                </div>
...
// ... (Rest of the file remains exactly the same: DeviceIOView, LinksVisual, DescriptorEssence, RAIDVisualizer, JournalingDemo, FileView)
// To ensure the file is complete, I will include the rest of the components unchanged.
...
const DeviceIOView = () => {
  ...
  const startSimulation = () => {
    ...
    if (method === 'polling') {
      // ... same polling logic ...
      setCpuStatus('busy');
      setCpuTask("while(!ready) check();");
      intervalRef.current = setInterval(() => {
        p += 5;
        if (p % 20 === 0) addLog("CPU: 设备就绪，读取一个字...");
        if (p >= 100) {
          p = 100;
          setIsRunning(false);
          setCpuStatus('idle');
  ...
  return (
    <div className="flex flex-col gap-6 h-full">
       <div className={`${styles.card} p-4 flex flex-col md:flex-row gap-6 shrink-0 items-center justify-between`}>
          ...
          <div className={`flex p-1 rounded-xl border ${mode === 'cute' ? 'bg-white border-pink-100' : 'bg-slate-200 border-slate-300'}`}>
             <button onClick={() => { setMethod('polling'); reset(); }} className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${method === 'polling' ? (mode === 'cute' ? 'bg-pink-400 text-white' : 'bg-slate-800 text-white') : 'text-slate-500'}`}>1. 轮询 (Polling)</button>
             <button onClick={() => { setMethod('interrupt'); reset(); }} className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${method === 'interrupt' ? (mode === 'cute' ? 'bg-pink-400 text-white' : 'bg-slate-800 text-white') : 'text-slate-500'}`}>2. 中断 (Interrupt)</button>
             <button onClick={() => { setMethod('dma'); reset(); }} className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${method === 'dma' ? (mode === 'cute' ? 'bg-pink-400 text-white' : 'bg-slate-800 text-white') : 'text-slate-500'}`}>3. DMA (Direct Memory Access)</button>
          </div>
       </div>
       <div className="flex-1 flex flex-col lg:flex-row gap-8 min-h-[400px]">
          <div className={`${styles.card} flex-1 p-8 relative flex flex-col justify-between overflow-hidden bg-slate-50/50`}>
             <div className="flex justify-center mb-12 relative z-10">
                <div className={`w-48 p-4 rounded-2xl border-4 transition-all duration-300 flex flex-col items-center gap-2 shadow-lg ${cpuStatus === 'busy' ? 'bg-orange-100 border-orange-400' : cpuStatus === 'user' ? 'bg-green-100 border-green-400' : cpuStatus === 'interrupt' ? 'bg-red-100 border-red-500 animate-bounce' : 'bg-slate-100 border-slate-300'}`}>
             ...
             <div className="flex justify-between items-end relative z-10">
                ...
                <div className="w-40 h-48 rounded-2xl border-4 border-emerald-200 bg-emerald-50 flex flex-col relative shadow-sm">
                   ...
                   <div className="p-2 text-center text-xs font-bold text-emerald-700 border-t border-emerald-200 bg-emerald-100 rounded-b-xl">Buffer</div>
                </div>
             </div>
             <svg className="absolute inset-0 w-full h-full pointer-events-none" style={{zIndex: 0}}>
                ...
                {isRunning && (
                   <>
                     {method === 'polling' && (
                        <>
                          <circle r="4" fill="#3b82f6"><animateMotion dur="1s" repeatCount="indefinite" path="M 20% 70% L 20% 50% L 50% 50% L 50% 120" /></circle>
                          <circle r="4" fill="#10b981"><animateMotion dur="1s" repeatCount="indefinite" begin="0.5s" path="M 50% 120 L 50% 50% L 80% 50% L 80% 65%" /></circle>
                        </>
                     )}
                     {method === 'interrupt' && cpuStatus === 'interrupt' && (
                        <>
                          <circle r="4" fill="#ef4444"><animateMotion dur="0.3s" repeatCount="indefinite" path="M 20% 70% L 20% 50% L 50% 50% L 50% 120" /></circle>
                          <circle r="4" fill="#ef4444"><animateMotion dur="0.3s" repeatCount="indefinite" begin="0.15s" path="M 50% 120 L 50% 50% L 80% 50% L 80% 65%" /></circle>
                        </>
                     )}
                     {method === 'dma' && (
                        <circle r="4" fill="#a855f7"><animateMotion dur="0.5s" repeatCount="indefinite" path="M 25% 80% Q 50% 90% 75% 80%" /></circle>
                     )}
                   </>
                )}
             </svg>
          ...
          <div className="w-full lg:w-72 flex flex-col gap-6">
             <div className={`${styles.card} p-5 flex flex-col`}>
                <div className="flex justify-center mb-4">
                   <button onClick={startSimulation} disabled={isRunning} className={`${styles.button.primary} w-full py-3 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed font-bold text-sm shadow-lg`}>
...
const LinksVisual = () => {
  ...
  const reset = () => { setIsDeleted(false); setSelected(null); };
  return (
    <div className="flex flex-col gap-6 h-full">
      <div className={`${styles.card} p-10 flex flex-col items-center justify-center relative overflow-hidden min-h-[400px]`}>
        <div className={`flex gap-12 items-start relative z-10 transition-opacity duration-500 ${isDeleted ? 'opacity-40' : 'opacity-100'}`}>
          ...
          <div className="flex flex-col justify-center gap-16 pt-12">
            <div className="flex items-center"><div className="h-0.5 w-20 bg-blue-300 relative"><div className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-1/2 w-2 h-2 bg-blue-400 rounded-full"></div></div></div>
            {selected && (<div className="flex items-center animate-in fade-in slide-in-from-left-4 duration-500"><div className={`h-0.5 w-20 relative ${selected === 'hard' ? 'bg-blue-400' : 'bg-rose-300 border-dashed border-t-2 bg-transparent'}`}><div className={`absolute right-0 top-1/2 -translate-y-1/2 translate-x-1/2 w-2 h-2 rounded-full ${selected === 'hard' ? 'bg-blue-400' : 'bg-rose-400'}`}></div></div></div>)}
          </div>
          <div className="flex flex-col gap-12 items-center">
            <div className={`p-6 rounded-[2.5rem] border-4 flex flex-col items-center gap-2 transition-all shadow-xl ${mode === 'cute' ? 'bg-indigo-50 border-indigo-200' : 'bg-blue-50 border-blue-200'}`}>
               ...
               <div className="text-center">
                  ...
                  <div className="text-[10px] font-bold mt-2 bg-indigo-200 text-indigo-700 px-3 py-1 rounded-full">引用计数: {isDeleted ? (selected === 'hard' ? '1' : '0') : (selected === 'hard' ? '2' : '1')}</div>
               </div>
            </div>
          </div>
          {selected && (
            <div className="flex flex-col gap-4 items-center animate-in slide-in-from-right-8 duration-500">
              ...
              <div className={`p-5 rounded-2xl border-2 flex flex-col items-center gap-2 ${selected === 'hard' ? 'bg-blue-50 border-blue-200' : 'bg-rose-50 border-rose-200'}`}>
                ...
                <div className={`text-[10px] px-2 py-0.5 rounded font-mono ${selected === 'hard' ? 'bg-slate-800 text-white' : 'bg-rose-500 text-white'}`}>{selected === 'hard' ? 'Inode: 42' : 'Inode: 99'}</div>
              </div>
              {selected === 'soft' && (<div className="text-[10px] text-rose-500 italic mt-1 bg-rose-100/50 px-2 py-1 rounded">指向路径: "/original.txt"</div>)}
            </div>
          )}
        </div>
        {isDeleted && (
          <div className="absolute inset-0 flex items-center justify-center bg-white/60 backdrop-blur-[2px] z-20 animate-in fade-in duration-700">
             <div className="text-center p-8 bg-white shadow-2xl rounded-3xl border-2 border-slate-100 flex flex-col items-center gap-3">
                ...
                <p className="text-sm text-slate-500 max-w-xs">{selected === 'hard' ? "由于 Inode 引用计数仍为 1，数据依然保留，硬链接有效。" : "软链接指向的路径已不存在，该链接变为‘死链接’。"}</p>
...
const DescriptorEssence = () => {
  ...
  const fdTable = [{ fd: 0, name: 'stdin', tableIdx: 0 }, { fd: 1, name: 'stdout', tableIdx: 0 }, { fd: 2, name: 'stderr', tableIdx: 0 }, { fd: 3, name: 'user_file.txt', tableIdx: 1 }];
  const openFileTable = [{ idx: 0, mode: 'rw', offset: 0, refCount: 3, inode: 10 }, { idx: 1, mode: 'r', offset: 1024, refCount: 1, inode: 42 }];
  const inodeTable = [{ inode: 10, type: 'CHR', size: 0, perms: 'crw--' }, { inode: 42, type: 'REG', size: 4096, perms: 'rw-r--' }];
  return (
    <div className="flex flex-col gap-6 h-full">
      <div className={`${styles.card} p-6`}>
         <h3 className={`font-bold text-lg ${styles.text.primary} mb-2`}>文件描述符 (File Descriptors)</h3>
         <p className={`text-sm ${styles.text.secondary}`}>Unix/Linux 系统中“一切皆文件”。进程通过文件描述符 (FD) 来访问文件。FD 只是一个整数索引，指向内核维护的打开文件表，进而指向 Inode。</p>
      </div>
      <div className="flex-1 flex flex-col md:flex-row gap-4 items-stretch justify-center min-h-[400px]">
        <div className="flex-1 flex flex-col">
          ...
          <div className={`${styles.card} flex-1 p-4 flex flex-col gap-2`}>
             {fdTable.map((entry) => (
               <div key={entry.fd} onMouseEnter={() => setHoverFd(entry.fd)} onMouseLeave={() => setHoverFd(null)} className={`p-3 rounded-lg border flex justify-between items-center cursor-pointer transition-all ${hoverFd === entry.fd ? (mode === 'cute' ? 'bg-pink-100 border-pink-300' : 'bg-blue-100 border-blue-400') : 'bg-white border-slate-200'}`}>
                 <div className="flex items-center gap-2"><div className="w-6 h-6 bg-slate-800 text-white rounded flex items-center justify-center font-mono text-xs">{entry.fd}</div><span className="text-xs font-bold">{entry.name}</span></div>
        ...
        <div className="flex-1 flex flex-col">
          ...
          <div className={`${styles.card} flex-1 p-4 flex flex-col gap-2 justify-center bg-slate-50/50`}>
             {openFileTable.map((entry) => {
               ...
               return (
                 <div key={entry.idx} className={`p-4 rounded-xl border-2 flex flex-col gap-1 transition-all ${isHighlighted ? (mode === 'cute' ? 'bg-purple-100 border-purple-300 scale-105' : 'bg-indigo-100 border-indigo-400 scale-105') : 'bg-white border-slate-200'}`}>
                   <div className="text-[10px] font-bold uppercase text-slate-400">Entry #{entry.idx}</div>
                   <div className="flex justify-between text-xs"><span>Offset: {entry.offset}</span><span className="font-mono bg-slate-200 px-1 rounded">Ref: {entry.refCount}</span></div>
                   <div className="flex justify-between text-xs items-center mt-1"><span className="font-bold text-slate-600">Inode: {entry.inode}</span><ArrowRight size={14} className="text-slate-400"/></div>
                 </div>
               );
             })}
          </div>
        ...
        <div className="flex-1 flex flex-col">
           ...
           <div className={`${styles.card} flex-1 p-4 flex flex-col gap-2 justify-center bg-slate-100/50`}>
              {inodeTable.map((node) => {
                 ...
                 return (
                   <div key={node.inode} className={`p-4 rounded-xl border-2 flex flex-col gap-1 transition-all ${isHighlighted ? (mode === 'cute' ? 'bg-orange-100 border-orange-300 scale-105' : 'bg-emerald-100 border-emerald-400 scale-105') : 'bg-white border-slate-200'}`}>
                     <div className="flex items-center gap-2 mb-1"><Database size={16} className="text-slate-400"/><span className="font-bold text-sm">Inode {node.inode}</span></div>
                     <div className="text-[10px] grid grid-cols-2 gap-x-2 text-slate-500"><span>Type: {node.type}</span><span>Size: {node.size}</span><span>Perms: {node.perms}</span></div>
                   </div>
                 );
              })}
           </div>
...
const RAIDVisualizer = () => {
  ...
  const renderDisks = () => {
    ...
    if (level === 0) { disks = [[], []]; blocks.forEach((b, i) => disks[i % 2].push(b)); } 
    else if (level === 1) { disks = [[], []]; blocks.forEach(b => { disks[0].push(b); disks[1].push(b); }); } 
    else if (level === 5) { disks = [[], [], []]; const pattern = [['A', 'B', 'P'], ['C', 'P', 'D'], ['P', 'E', 'F']]; pattern.forEach(row => { disks[0].push(row[0]); disks[1].push(row[1]); disks[2].push(row[2]); }); }
    return (
          ...
          <div key={dIdx} className="flex flex-col items-center">
             <div className="w-32 h-40 bg-slate-800 rounded-lg p-2 flex flex-col gap-2 border-b-4 border-slate-600 shadow-xl relative">
                ...
                {diskContent.map((block, bIdx) => (
                  <div key={bIdx} className={`h-8 rounded flex items-center justify-center font-bold text-sm shadow-sm transition-all animate-in zoom-in duration-500 delay-${bIdx * 100} ${block?.startsWith('P') ? 'bg-purple-500 text-white border border-purple-400' : (mode === 'cute' ? 'bg-pink-100 text-pink-600 border border-pink-200' : 'bg-blue-100 text-blue-700 border border-blue-200')}`}>{block}</div>
                ))}
             </div>
             <div className="mt-2 w-24 h-2 rounded-full bg-slate-200 overflow-hidden"><div className="h-full bg-green-500 w-1/3 animate-ping" style={{ animationDuration: '2s' }}></div></div>
          </div>
        ))}
      </div>
    );
    ...
    <div className="flex flex-col h-full gap-6">
       <div className={`${styles.card} p-6 flex flex-col items-center`}>
          ...
          <div className="flex gap-4 mb-6">
            <button onClick={() => setLevel(0)} className={`px-6 py-3 rounded-xl border-2 font-bold transition-all ${level === 0 ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-slate-200 hover:bg-slate-50'}`}>RAID 0 (Striping)</button>
            <button onClick={() => setLevel(1)} className={`px-6 py-3 rounded-xl border-2 font-bold transition-all ${level === 1 ? 'border-green-500 bg-green-50 text-green-700' : 'border-slate-200 hover:bg-slate-50'}`}>RAID 1 (Mirroring)</button>
            <button onClick={() => setLevel(5)} className={`px-6 py-3 rounded-xl border-2 font-bold transition-all ${level === 5 ? 'border-purple-500 bg-purple-50 text-purple-700' : 'border-slate-200 hover:bg-slate-50'}`}>RAID 5 (Parity)</button>
          </div>
       ...
       <div className="flex-1 overflow-y-auto">{renderDisks()}</div>
    </div>
  );
};
...
const JournalingDemo = () => {
  ...
  const steps = [{ title: "Idle", desc: "系统空闲。" }, { title: "Tx Begin", desc: "开始事务 (Transaction Begin)。记录到日志区。" }, { title: "Journal Write", desc: "将元数据和数据写入日志区域。" }, { title: "Tx End (Commit)", desc: "写入事务结束标记 (Commit Block)。此时事务被视为“已提交”。" }, { title: "Checkpoint", desc: "将数据写入实际文件系统位置 (Checkpointing)。" }, { title: "Free", desc: "释放日志空间。" }];
  const nextStep = () => {
    if (crashed) {
      if (step >= 3) { alert("检测到已提交的事务！重放日志 (Replay)... 恢复数据！"); setStep(5); } 
      else { alert("检测到未提交的事务。丢弃日志... 数据未损坏（保持旧状态）。"); setStep(0); }
      setCrashed(false); return;
    }
    setStep((prev) => (prev + 1) % steps.length);
  };
  const simulateCrash = () => { setCrashed(true); };
  return (
    <div className="flex flex-col h-full gap-6 p-4">
       <div className={`${styles.card} p-6`}>
         <div className="flex justify-between items-center">
            <div><h3 className={`font-bold text-lg ${styles.text.primary}`}>日志文件系统 (Journaling)</h3><p className="text-sm text-slate-500">模拟写入过程及崩溃恢复 (Crash Consistency)</p></div>
            <div className="flex gap-2">
               <button onClick={simulateCrash} disabled={crashed || step === 0 || step === 5} className="bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-lg font-bold shadow-sm disabled:opacity-50 transition-colors"><Zap size={16} className="inline mr-1"/> 模拟断电崩溃</button>
               <button onClick={nextStep} className={`${styles.button.primary} px-6 py-2 flex items-center gap-2`}>{crashed ? <RotateCcw size={16}/> : <ArrowRight size={16}/>}{crashed ? "重启并恢复 (Recovery)" : "下一步"}</button>
            </div>
         </div>
       </div>
       <div className="flex-1 flex flex-col items-center justify-center gap-12 relative">
          <div className="w-full max-w-3xl flex items-center justify-between relative px-4">
             ...
             {steps.map((s, i) => (
               <div key={i} className={`flex flex-col items-center gap-2 transition-all ${i === step ? 'scale-110' : 'opacity-60'}`}>
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs border-2 z-10 transition-colors ${i <= step ? (crashed && i === step ? 'bg-red-500 border-red-600 text-white animate-pulse' : 'bg-green-500 border-green-600 text-white') : 'bg-white border-slate-300 text-slate-400'}`}>{i + 1}</div>
          ...
          <div className="flex gap-12 items-stretch h-64">
             <div className={`w-64 rounded-xl border-4 p-4 flex flex-col gap-2 transition-all ${step > 0 && step < 5 ? 'border-orange-400 bg-orange-50 shadow-lg' : 'border-slate-300 bg-slate-50 opacity-50'}`}>
                ...
                {step >= 2 && <div className="bg-white p-2 rounded text-xs border border-orange-200 shadow-sm"><div>Metadata: Inode #42</div><div>Data: Block #99</div></div>}
             ...
             <div className="flex items-center justify-center text-slate-300"><ArrowRight size={32} className={step === 4 ? 'text-green-500 animate-pulse' : ''}/></div>
             <div className={`w-64 rounded-xl border-4 p-4 flex flex-col gap-2 transition-all ${step === 4 ? 'border-green-400 bg-green-50 shadow-lg' : 'border-slate-300 bg-slate-50'}`}>
                ...
                {step >= 4 ? (<div className="flex-1 flex flex-col justify-center gap-2 animate-in zoom-in"><div className="bg-white p-2 rounded text-xs border border-green-200 shadow-sm">Updated Inode #42</div><div className="bg-white p-2 rounded text-xs border border-green-200 shadow-sm">Updated Block #99</div><div className="text-center text-xs text-green-600 font-bold mt-2"><CheckCircle size={14} className="inline"/> Checkpoint Done</div></div>) : (<div className="flex-1 flex items-center justify-center text-slate-400 text-xs italic">Old State</div>)}
             </div>
          </div>
          <div className="max-w-xl text-center p-4 bg-slate-100 rounded-xl text-sm text-slate-600">{crashed ? "⚡ 系统崩溃！电源中断..." : steps[step].desc}</div>
       </div>
    </div>
  );
};
...
const TabButton = ({ active, onClick, label, mode, icon }: any) => (
  <button onClick={onClick} className={`px-6 py-2 rounded-xl font-bold text-xs transition-all flex items-center gap-2 ${active ? (mode === 'cute' ? 'bg-pink-400 text-white shadow-md' : 'bg-white text-slate-800 shadow-sm') : 'text-slate-400 hover:text-slate-600'}`}>{icon} {label}</button>
);
